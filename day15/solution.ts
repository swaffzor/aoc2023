...
export const part1 = (input: string) => {
  ...
  const result = steps.reduce((acc, step) => {
    const hash = hashIt(step)
    const current = acc + hash
    return current
  }, 0)
  return result
}

export const hashIt = (input: string, part2 = false) => {
  let current = 0
  const chars = part2 ? input.split(/[-=]/)[0].split('') : input.split('')
  chars.forEach((char) => {
...
export const part2 = (input: string) => {
  ...
  const result = boxes.reduce((acc, box, boxIndex) => {
    const current = acc + calcResult(box, boxIndex)
    return current
  }, 0)
  return result
}

export const calcResult = (box: string[], boxNum: number) => {
  return box.reduce((acc, lens, index) => {
    const [label, focalLength] = lens.split(' ')
    const current = acc + (boxNum + 1) * (index + 1) * parseInt(focalLength, 10)
    return current
  }, 0)
}
