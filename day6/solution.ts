...
export const part1 = (input: string): number => {
  ...
  const numberOfFinishes = data.map(({ time, distance }) => {
    const distances = calculateDistances(time)
    return distances.filter((d) => d > distance).length
  })
  return numberOfFinishes.reduce((acc, n) => acc * n)
}

export const calculateDistances = (time: number): number[] => {
  const distances = []
  for (let i = 0; i < time; i++) {
    const velocity = i
    const distance = (time - i) * velocity
    distances.push(distance)
  }
  return distances
}

export const calculateNumberOfWaysToWin = (
  races: Race[],
  distance: number
): number => {
