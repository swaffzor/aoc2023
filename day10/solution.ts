...
const extractDataToPointGrid = <T>(input: string) => {
  const grid: Point<T>[][] = []
...
export const part1 = (input: string) => {
  // ;['|', '-', 'L', 'J', '7', 'F'].forEach((char) => {
  //   console.log(char, ':', replaceWithAscii(char as Pipe))
  // })
  ...
  const pass1 = calcDistances(JSON.parse(JSON.stringify(connectedGrid)), {
    ...start,
    east: undefined,
    west: undefined,
    north: undefined,
  ...
  pass1
    ...
    .forEach((point, rowIndex) => {
      if (row !== point.row) {
        // process.stdout.write('\n')
        row = point.row
      }
      // process.stdout.write(`${point.distance >= 0 ? point.value : '.'}`)
  ...
  pass1
    ...
    .forEach((point, rowIndex) => {
      ...
      if (point.distance === -1) {
        const neighbors = getPointNeighbors(point, pass1, false)
        const touchesPipe = neighbors.some(
          (neighbor) => pass1[neighbor.row][neighbor.col].distance >= 0
        )

        // if touchesPipe, get the neighbors and see if any of them are pipes

        point = {
          ...point,
          value: touchesPipe ? ('O' as Pipe) : point.value,
        }
      }
      // process.stdout.write(`${point.distance >= 0 ? ' ' : point.value}`)
      return point.distance === -1
  ...
  // const oof = remaining.map((point) => {
  //   const neighbors = getPointNeighbors(point, pass1, false)
  //   const tempp = neighbors.some(
  //     (neighbor) => pass1[neighbor.row][neighbor.col].distance >= 0
  //   )
  //   return {
  //     ...point,
  //     value: tempp ? 'O' : point.value,
  //   }
  // })
  // console.log(oof.map((o) => o.value).join(''))
  return pass1
}
// 13639 is too high
// 6820 is correct
...
export const determineConnection = (
  ...
  return {
    ...result,
    distance: -1,
    // value: replaceWithAscii(result.value) as Pipe,
  }
}

const replaceWithAscii = (char: Pipe) => {
  switch (char) {
    // case '|':
    //   return char.replace('|', String.fromCharCode(186))
    case '7':
      return char.replace('7', '⅂')
    case 'J':
      return char.replace('J', '⅃')
    // case 'L':
    //   return char.replace('L', String.fromCharCode(200))
    case 'F':
      return char.replace('F', '')
    case '-':
      return char.replace('-', String.fromCharCode(173))
    default:
      return char
  }
}
...
// part1(getPuzzleInput('sampleInput', 'day10'))
export const doTheWork = (useSample: number) =>
  part1(
    useSample % 3 === 0
      ? sampleInput21
      : useSample % 2 == 0
...
// part1(getPuzzleInput('input', 'day10'))

const sampleInput21 = `.F----7F7F7F7F-7....
.|F--7||||||||FJ....
.||.FJ||||||||L7....
FJL7L7LJLJ||LJ.L-7..
L--J.L7...LJS7F-7L7.
...
....FJL-7.||.||||...
....L---J.LJ.LJLJ...`
const sampleInput22 = `FF7FSF7F7F7F7F7F---7
L|LJ||||||||||||F--J
FL-7LJLJ||||||LJL-77
F--JF--7||LJLJ7F7FJ-
L---JF-JLJ.||-FJLJJ7
...
L.L7LFJ|||||FJL7||LJ
L7JLJL-JLJLJL--JLJ.L`
const puzzleInput = `7.FF7777.|77.-L-77.|F-7-.L77|7F|7-|-JF77FFF777.7FL--F-F..F.-7-F7-J.FFF7JFF7-J7....FFJFL.|-L7F--|-L7L-|-|-|.F77F7F-|7---L-.7-.F-7F|-L77...-.-
